    ...
    if (new_mode == VIEW_MODE__3D) {
        ...
        if (render_state.dest_handle == 0) {
            ...
            gluPerspective (fov, (GLfloat) dst_w / (GLfloat) dst_h, 0.1,
                            10000.0); // Set the Field of view angle (in degrees), the aspect ratio of our window, and the new and far planes
            glMatrixMode (GL_MODELVIEW);
            glLoadIdentity();
...
void hardware_img_tri3d (float dst_x1, float dst_y1, float dst_z1, float dst_x2, float dst_y2, float dst_z2,
...
void GLUT_DISPLAY_REQUEST()
{
    ...
    if (i != last_i) {
        for (i2 = 0; i2 <= 2; i2++) {
            if (display_frame[i2].order < display_frame[i].order && (display_frame[i2].state == DISPLAY_FRAME_STATE__DISPLAYING
            ...
            if (level == displayorder_hardware || level == displayorder_hardware1) {
                    ...
                    if (first_hardware_command) {
                        ...
                        if (last_hardware_command_rendered) {
                            ...
                            if (rerender_prev_hardware_frame) {
                                ...
                                hardware_graphics_command_struct *last_hgc = (hardware_graphics_command_struct *) list_get (
                                            hardware_graphics_command_handles, last_hardware_command_rendered);
                        ...
                        while (command) {
                            ...
                            if (hgc->order < order) {
                                ...
                                if (hgc->command == HARDWARE_GRAPHICS_COMMAND__PUTIMAGE) {
                                    if (hgc->dst_img >
                                            0) { //note: rendering to the old default surface is pointless, but renders onto maintained hardware images are still required
                ...
                while (command) {
                    ...
                    if (hgc->order == order) {
                        ...
                        if (hgc->command == HARDWARE_GRAPHICS_COMMAND__FREEIMAGE && rerender_prev_hardware_frame == 0
                                && first_hardware_layer_rendered == 0) {
            ...
            if (level == 5) {
                ...
                if (environment_2d__letterbox) {
                    ...
                    if (environment_2d__letterbox == 1) {
                        //vertical stripes
                        hardware_img_put ( ( (float) - environment_2d__screen_x1) / environment_2d__screen_x_scale - 1.0f, 0, -1,
                        ...
                        hardware_img_put (environment_2d__screen_width, 0,
                                          ...
                                          1.0f, environment_2d__screen_height - 1,
                                          black_texture, 0,
                                          0, 0, 0, 0,
                                          0, 0);
                        ...
                        //horizontal stripes
                        hardware_img_put (0, ( (float) - environment_2d__screen_y1) / environment_2d__screen_y_scale - 1.0f,
                                          environment_2d__screen_width - 1, -1,
                                          black_texture, 0,
                                          0, 0, 0, 0,
                                          0, 0);
