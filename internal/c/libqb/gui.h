...
void hardware_img_buffer_to_texture (int32 handle)
{
    ...
    if (hardware_img->texture_handle == 0) {
        ...
        if (force_NPO2_fix == 0) {
            glTexImage2D (GL_TEXTURE_2D, 0, GL_RGBA, hardware_img->w, hardware_img->h, 0, GL_BGRA, GL_UNSIGNED_BYTE,
        ...
        if (glerrorcode != 0 || force_NPO2_fix == 1) {
            ...
            if (glerrorcode) {
                gluBuild2DMipmaps (GL_TEXTURE_2D, GL_RGBA, hardware_img->w, hardware_img->h, GL_BGRA, GL_UNSIGNED_BYTE,
                                   hardware_img->software_pixel_buffer);
                glerrorcode = glGetError();
...
void hardware_img_requires_depthbuffer (hardware_img_struct *hardware_img)
{
    if (hardware_img->depthbuffer_handle == 0) {
        ...
        glTexImage2D (GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, hardware_img->w, hardware_img->h, 0, GL_DEPTH_COMPONENT,
