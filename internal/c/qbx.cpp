...
extern void sub__maptriangle (int32 cull_options, float sx1, float sy1, float sx2, float sy2, float sx3, float sy3,
                              int32 si, float dx1, float dy1, float dz1, float dx2, float dy2, float dz2, float dx3, float dy3, float dz3, int32 di,
...
extern void sub__putimage (double f_dx1, double f_dy1, double f_dx2, double f_dy2, int32 src, int32 dst, double f_sx1,
                           double f_sy1, double f_sx2, double f_sy2, int32 passed);
...
extern void qbg_screen (int32 mode, int32 color_switch, int32 active_page, int32 visual_page, int32 refresh,
                        int32 passed);
...
extern void sub_circle (double x, double y, double r, uint32 col, double start, double end, double aspect,
                        int32 passed);
...
template <typename T> static T qbs_cleanup (uint32 base, T passvalue)
{
    while (qbs_tmp_list_nexti > base) {
        qbs_tmp_list_nexti--;    //clear any temp. strings created
...
inline uint8 func_abs (uint8 d)
{
    return d;
}
inline uint16 func_abs (uint16 d)
{
    return d;
}
inline uint32 func_abs (uint32 d)
{
    return d;
}
inline uint64 func_abs (uint64 d)
{
    return d;
}
inline int8 func_abs (int8 d)
{
    return abs (d);
}
inline int16 func_abs (int16 d)
{
    return abs (d);
}
inline int32 func_abs (int32 d)
{
    return abs (d);
}
inline int64 func_abs (int64 d)
{
    return llabs (d);
}
...
void chain_input()
{
    ...
    if ( (func_command (0, 0) )->len >= 32) {
        if (qbs_equal (qbs_right (func_command (0, 0), 4), qbs_new_txt_len (".tmp", 4) ) ) {
            if (qbs_equal (func_mid (func_command (0, 0), (func_command (0, 0) )->len - 31, 25, 1),
...
void sub_chain (qbs *f)
{
    ...
    if (!f->len) {
        error (53);   //file not found (as in QB)
        return;
    }
...
{
    return * (device->events + eventIndex * device->event_size + device->lastaxis * 4 + device->lastwheel * 4 +
              objectIndex);
}
void setDeviceEventButtonValue (device_struct *device, int32 eventIndex, int32 objectIndex, uint8 value)
{
    * (device->events + eventIndex * device->event_size + device->lastaxis * 4 + device->lastwheel * 4 + objectIndex) =
        value;
}
...
int32 createDeviceEvent (device_struct *device)
{
    ...
    memmove (device->events + device->queued_events * device->event_size,
             device->events + (device->queued_events - 1) *device->event_size, device->event_size);
    * (int64 *) (device->events + (device->queued_events * device->event_size) + (device->event_size - 8) ) =
...
void onstrig_setup (int32 i, int32 controller, int32 controller_passed, uint32 id, int64 pass)
{
    ...
    if (i & 1) {
        error (5);   //'currently down' state cannot be used as an ON STRIG event
        return;
    }
                            ...
                            if (!ontimer[i].state) {
                                if (time_now - ontimer[i].last_time > ontimer[i].seconds) {
                                    if (!ontimer[i].last_time) {
                                        ...
                                        if (fabs (time_now - ontimer[i].last_time) >= ontimer[i].seconds) {
                                            ontimer[i].last_time = time_now;
                                        }
...
void evnt (uint32 linenumber, uint32 inclinenumber = 0, const char *incfilename = NULL)
{
    ...
    if (sub_gl_called == 0) {
        if (display_lock_request > display_lock_confirmed) {
            ...
            while ( (display_lock_released < display_lock_confirmed) && (!close_program) && (!suspend_program)
                    && (!stop_program) ) {
                Sleep (1);
            }
        }
...
void QBMAIN (void *unused)
{
    ...
    */
    ...
    if (!controlfp_set) {
        controlfp_set = 1;    //_MCW_PC=0x00030000
        _controlfp (_PC_64, 0x00030000);
    }
