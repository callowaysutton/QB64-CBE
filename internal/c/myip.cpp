...
qbs *WHATISMYIP()  //changed name from FUNC_WHATISMYIP to WHATISMYIP
{
    ...
    if (!byte_element_5276) {
        if ( (mem_static_pointer += 12) < mem_static_limit) {
            byte_element_5276 = (byte_element_struct *) (mem_static_pointer - 12);

        } else {
            byte_element_5276 = (byte_element_struct *) mem_static_malloc (12);
        }
    }
    ...
    if (!byte_element_5280) {
        if ( (mem_static_pointer += 12) < mem_static_limit) {
            byte_element_5280 = (byte_element_struct *) (mem_static_pointer - 12);

        } else {
            byte_element_5280 = (byte_element_struct *) mem_static_malloc (12);
        }
    }
    ...
    if (!_FUNC_WHATISMYIP_STRING_IP) {
        _FUNC_WHATISMYIP_STRING_IP = qbs_new (0, 0);
    }
    ...
    *_FUNC_WHATISMYIP_SINGLE_C = func__openclient (qbs_new_txt_len ("TCP/IP:80:www.qb64.net", 22) );
    ...
    qbs_cleanup (qbs_tmp_base, 0);
    qbs_set (_FUNC_WHATISMYIP_STRING_X, qbs_add (qbs_add (_FUNC_WHATISMYIP_STRING_X, qbs_new_txt_len ("Host: www.qb64.net",
             18) ), _FUNC_WHATISMYIP_STRING_E) );
    qbs_cleanup (qbs_tmp_base, 0);
    qbs_set (_FUNC_WHATISMYIP_STRING_X, qbs_add (qbs_add (_FUNC_WHATISMYIP_STRING_X, qbs_new_txt_len ("", 0) ),
             _FUNC_WHATISMYIP_STRING_E) );
    qbs_cleanup (qbs_tmp_base, 0);
    sub_put2 (qbr (*_FUNC_WHATISMYIP_SINGLE_C), NULL, byte_element ( (uint64) _FUNC_WHATISMYIP_STRING_X->chr,
              _FUNC_WHATISMYIP_STRING_X->len, byte_element_5276), 0);
    qbs_cleanup (qbs_tmp_base, 0);
    *_FUNC_WHATISMYIP_SINGLE_T = func_timer (NULL, 0);
    ...
    do {
        ...
        *_FUNC_WHATISMYIP_SINGLE_START = 0 ;
        ...
        if (fornext_step5279 < 0) {
            fornext_step_negative5279 = 1;

        } else {
            fornext_step_negative5279 = 0;
        }

        if (new_error) {
            goto fornext_error5279;
        }
        ...
        fornext_entrylabel5279:
            *_FUNC_WHATISMYIP_SINGLE_X = fornext_value5279;
            ...
            if (fornext_step_negative5279) {
                if (fornext_value5279 < fornext_finalvalue5279) {
                    break;
                }
                ...
                if ( ( (- (*_FUNC_WHATISMYIP_SINGLE_A == 46) ) & (- (*_FUNC_WHATISMYIP_SINGLE_START != 0) ) ) || new_error) {
                    *_FUNC_WHATISMYIP_SINGLE_DOTS = *_FUNC_WHATISMYIP_SINGLE_DOTS + 1 ;
                    ...
                    if ( (- (*_FUNC_WHATISMYIP_SINGLE_DOTS == 3) ) || new_error) {
                        qbs_set (_FUNC_WHATISMYIP_STRING_IP, func_mid (_FUNC_WHATISMYIP_STRING_A, qbr (*_FUNC_WHATISMYIP_SINGLE_START),
                                 qbr (*_FUNC_WHATISMYIP_SINGLE_X - *_FUNC_WHATISMYIP_SINGLE_START), 1) );
                        qbs_cleanup (qbs_tmp_base, 0);
                        goto dl_exit_5277;
                    }
...
exit_subfunc:
    ...
    if ( (tmp_mem_static_pointer >= mem_static) && (tmp_mem_static_pointer <= mem_static_limit) ) {
